...
**Hermes Agent 专用增长决策插件** — 在 Hermes Agent 中直接调用专业的增长策略分析能力。
...
- Python 3.8+ 已安装
- 知识库文件已复制到 `~/.hermes/skills/oh-my-growth/knowledge/`
- 计算脚本已复制到 `~/.hermes/skills/oh-my-growth/scripts/`
...
| `assess` | 先判断能不能进入策略分析 | 现状还不够清楚，先做清晰度评估 |
| `design` | 可落地的策略 | 知道要做什么，需要执行路径 |
